...
abstract class ToolsBase
{
    ...
    final public function supportedDatabases(): array
    {
        static $dbs = [];
    ...
    {
        if (!empty(self::$script_name) && file_exists(Config::$boarddir . '/' . self::$script_name)) {
            if (!empty($_SESSION['ftp'])) {
                ...
                $ftp->unlink(self::$script_name);
                $ftp->close();

                unset($_SESSION['ftp']);  
            } else {
                @unlink(Config::$boarddir . '/' . self::$script_name);
            }
